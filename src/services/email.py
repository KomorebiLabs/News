...
class EmailManager:
    ...
    def send_daily_summary(self, summary_md: str, subject: str, subscribers: List[str]):
        ...
        if not self.config.enabled or not subscribers:
            self.console.print("[yellow]⚠️  Email enabled but no subscribers configured[/yellow]")
            return
            ...
            with smtplib.SMTP_SSL(
                ...
                for subscriber in subscribers:
                    ...
                    try:
                        ...
                        self.console.print(f"   [green]✓ Email sent to {subscriber}[/green]")
                    except Exception as e:
                        logger.error(f"Failed to send to {subscriber}: {e}")
                        self.console.print(f"   [red]✗ Failed to send to {subscriber}: {e}[/red]")

        except smtplib.SMTPAuthenticationError as e:
            logger.error(f"SMTP Authentication Error: {e}")
            self.console.print(f"[red]✗ SMTP login failed! Check your email password/authorization code in .env[/red]")
